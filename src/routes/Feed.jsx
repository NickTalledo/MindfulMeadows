...
const Feed = () => {
  ...
  const [searchField, setSearchField] = useState("disorder");

  const filteredPosts = posts.filter((post) => {
    if (!searchQuery) return true;
    if (searchField === "disorder") {
      return post.disorder.toLowerCase().includes(searchQuery.toLowerCase());
    }
            ...
            <Link to={`/post/${post.postid}`} key={post.postid}>
              <div className="mb-4 p-4 shadow rounded-lg hover:shadow-xl transition-shadow duration-300 ease-in-out bg-gray-100">
                <div className="flex justify-between">
                  <div>
                    <h2 className="text-2xl font-bold mb-2">{post.disorder}</h2>
                    <h3 className="text-xl font-semibold mb-4">{post.issue}</h3>
                  </div>
